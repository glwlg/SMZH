...
> 2026-05-08 状态校正：`v0.10-3D战场演示链` 是一次试验记录，当前仓库没有 `BattleStage3D` / `BattleUnitVisual3D` 脚本，也没有 `backup/` 路径下的 3D 贴图落地资产。当前可运行战斗仍以 `BattleStage2D` 和已登记的 2D/高清 Sprite 素材为准。

| 批次 | 资源范围 | 来源工具 | 生成说明 | 二次处理 |
| --- | --- | --- | --- | --- |
| v0.10-3D战场演示链（未落地） | 3D战场远景背景、战场地面、我方阵门、敌方魔门 | imagegen CLI / 本地 OpenAI 兼容接口（`gpt-image-2`） | 面向 2.5D/3D 战场演示链的环境补图；当前仓库未接入为可运行资产 | 是 |
...
本段为试验记录，不代表当前可运行实现。当前仓库实际使用 `BattleStage2D` 和 `Assets/Resources/Art/AI/Backgrounds/battlefield_honghuang_ai.png`。
